...
if df_atual.empty:
    ...
    with tab1:
        ...
        if MODO_OFFLINE:
            st.info(
                "📡 Voce esta offline. O scanner OCR pode nao funcionar sem internet "
                "(dependendo do ambiente, ele precisa baixar o modelo na primeira vez). "
                "Mas isso nao impede nada: **digite o numero do ativo manualmente** no campo "
                "logo abaixo e continue lancando o patrimonio normalmente — tudo e salvo no "
                "banco local e sincroniza sozinho quando a internet voltar."
            )
            ...
            with col2:
                if EASYOCR_DISPONIVEL:
                    try:
                        ...
                        # Falha no OCR (ex: sem internet pra baixar modelo na primeira vez)
                        # nunca deve travar a pagina - o lancamento manual continua disponivel.
                        st.warning(f"OCR indisponivel agora ({e}). Digite o codigo manualmente abaixo.")
                else:
                    st.info("OCR desabilitado.")

        st.markdown("---")
        st.markdown("### Confirmar ou Digitar Codigo Manualmente")
        st.caption("✅ Este campo funciona normalmente offline — o lancamento e salvo no banco local.")
