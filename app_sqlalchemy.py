...
# Timeout (segundos) do pre-check TCP rapido, usado para detectar "sem rede
# nenhuma" sem esperar o timeout longo (10-30s) da conexao completa com o banco.
TIMEOUT_PRECHECK_REDE = 1.0  # reduzido de 2.0 para acelerar detecção

# ============================================================
# CONFIGURAR OCR (EasyOCR) - CARREGAMENTO PREGUICOSO (LAZY)
# ============================================================
try:
    import easyocr
    EASYOCR_INSTALADO = True
except Exception:
    EASYOCR_INSTALADO = False
...
def _precheck_tcp_rapido(engine_remoto):
    resultado = {"ok": False}

    def _tentar():
        try:
            url = engine_remoto.url
            host = url.host
            porta = url.port or (5432 if "postgres" in url.drivername else 26257)
            if not host:
                resultado["ok"] = True
                return
            with socket.create_connection((host, porta), timeout=TIMEOUT_PRECHECK_REDE):
                resultado["ok"] = True
        except Exception:
            resultado["ok"] = False
    ...
    t.start()
    t.join(timeout=TIMEOUT_PRECHECK_REDE + 0.5)
    if t.is_alive():
        return False
    return resultado["ok"]


def _tentar_conexao_completa_com_timeout(engine_remoto, timeout_segundos=2.0):  # reduzido de 3.0
    ...
    if t.is_alive():
        return False, "Timeout ao tentar conectar na nuvem (driver nao respondeu a tempo)."
...
def testar_remoto(engine_remoto):
    ...
    if not _precheck_tcp_rapido(engine_remoto):
        return False, "Sem rede (pre-check TCP falhou - provavel ausencia de internet)."
    return _tentar_conexao_completa_com_timeout(engine_remoto, timeout_segundos=2.0)
...
def _inicializar_conexao():
    if "modo_offline" not in st.session_state:
        st.session_state.modo_offline = None
    ...
    try:
        with engine.begin() as conn:
            ...
            if is_sqlite:
                ...
                """))
            conn.execute(text("CREATE INDEX IF NOT EXISTS idx_inventario_obra_id ON inventario (obra_id)"))
            conn.execute(text("CREATE INDEX IF NOT EXISTS idx_inventario_codigo ON inventario (obra_id, codigo)"))
            return True
    except Exception as e:
        msg = str(e).lower()
        if "already exists" in msg or "ja existe" in msg or "duplicate" in msg:
            return True
...
# ============================================================
# INICIALIZAÇÃO DO BANCO DE DADOS (AGORA DENTRO DO SPINNER)
# ============================================================
# Declaração inicial (serão preenchidas dentro do spinner)
engine = None
MODO_OFFLINE = True
AVISO_POOL = None
erro_engine = None
...
# ============================================================
# INICIALIZAÇÃO DO BANCO (AGORA DENTRO DO SPINNER)
# ============================================================
with st.spinner("🔌 Conectando ao banco de dados..."):
    ...
    if engine is None:
        st.error(f"❌ Não foi possível iniciar nenhum banco de dados (nem CockroachDB, nem local): {erro_engine}")
        st.stop()
...
# OCR só faz sentido (e só é seguro tentar) quando o app está online
EASYOCR_DISPONIVEL = EASYOCR_INSTALADO and not MODO_OFFLINE


@st.cache_resource(show_spinner=False)
def _obter_leitor_ocr():
    try:
        import easyocr
...
if EASYOCR_DISPONIVEL:
    _EASYOCR_READER = _obter_leitor_ocr()
    if _EASYOCR_READER is None:
        EASYOCR_DISPONIVEL = False


# ============================================================
# MENSAGENS DE STATUS (MODO OFFLINE / SINCRONIZAÇÃO)
# ============================================================
if MODO_OFFLINE:
    pendentes_local = contar_pendentes_sync(engine) if engine is not None else 0
    st.markdown(
        '<div class="warning-box">⚠️ <strong>Modo Local (sem internet)</strong> — os dados estão sendo salvos '
    ...
    _erro_conexao_atual = st.session_state.get("ultimo_erro_conexao")
    if _erro_conexao_atual:
        with st.expander("Detalhes técnicos (por que está offline)"):
            st.code(_erro_conexao_atual)
else:
    ...
    if sync_info and sync_info.get("status") == "ok" and (sync_info.get("enviados", 0) > 0 or sync_info.get("exclusoes", 0) > 0):
        ...
        if sync_info.get("exclusoes", 0) > 0:
            partes.append(f"{sync_info['exclusoes']} exclusão(ões)")
        msg = f"☁️ Conexão com a nuvem restabelecida! {' e '.join(partes)} sincronizado(s)."
        if sync_info.get("erros", 0) > 0:
            msg += f" ({sync_info['erros']} falharam e serão tentados novamente)."
        st.success(msg)
        st.session_state.ultimo_sync_info = None
    elif sync_info and sync_info.get("status") == "erro":
        st.warning(f"⚠️ Conexão com a nuvem restabelecida, mas houve erro ao sincronizar dados locais: {sync_info.get('mensagem', '')}")
        st.session_state.ultimo_sync_info = None
...
# ============================================================
# INICIALIZAÇÃO MULTISESSAO / MULTIOBRA
# ============================================================
...
with st.sidebar:
    st.markdown("### Identificação da Obra")
    obras_existentes = listar_obras_banco()

    obra_input = st.text_input("ID da Obra:", value=obra_id_atual, key="obra_input_field", help="Use um ID único para cada obra")
    ...
    if arquivo is not None:
        try:
            ...
            if arquivo_hash in st.session_state.arquivos_processados:
                st.warning("⚠️ Este arquivo já foi importado anteriormente! Selecione outro arquivo.")
            else:
                ...
                if df_clean.empty:
                    ...
                    if resultado == False:
                        ...
                        st.session_state[db_key] = carregar_do_banco(obra_id_atual)
                        msg = f"✅ Importação concluída em '{obra_id_atual}':"
                    ...
                    if sucesso:
                        if db_key in st.session_state:
                            st.session_state[db_key] = pd.DataFrame(columns=["Codigo do Bem", "Descricao do Bem", "Status", 
                                                                               "Data Auditoria", "Observacoes", "Quantidade", "Local"])
                        st.session_state.confirmar_excluir_obra = False
                        st.success(f"✅ Obra '{obra_id_atual}' excluída!")
                        time.sleep(1)
                        st.rerun()
                    else:
    ...
    with st.expander("🚨 EXCLUIR TUDO (App + Banco)", expanded=False):
        st.error("⚠️ ATENÇÃO: Isso vai APAGAR TODOS os dados de TODAS as obras do BANCO e da MEMÓRIA do app!")
        if not st.session_state.confirmar_excluir_tudo:
            ...
            st.error("⚠️⚠️ CONFIRME NOVAMENTE - ESTA AÇÃO NÃO PODE SER DESFEITA")
            col1, col2 = st.columns(2)
            with col1:
                if st.button("✅ SIM, APAGAR TUDO", type="primary", key="btn_excluir_tudo_2"):
                    try:
                        ...
                        if sucesso:
                            st.session_state.confirmar_excluir_tudo = False
                            st.success("🗑️ TODOS os dados foram excluídos do banco e da memória!")
                            time.sleep(1)
                            st.rerun()
                        else:
                            st.error("Erro ao excluir do banco. Memória limpa.")
                            st.session_state.confirmar_excluir_tudo = False
                    except Exception as e:
                        st.error(f"Erro: {e}")
                        st.session_state.confirmar_excluir_tudo = False
...
if df_atual.empty:
    st.warning("Nenhum inventário carregado. Importe um Excel ou selecione uma obra existente.")
    st.info("""
    Como usar:
    1. Digite um ID de obra (ex: obra_001)
    2. Importe o Excel pela barra lateral
    3. Use a aba "Scanner e OCR" para auditar com foto
    4. Os dados são salvos automaticamente no CockroachDB
    """)
else:
    ...
    with tab1:
        ...
        if MODO_OFFLINE:
            st.warning(
                "📵 OCR desativado em modo offline (evita travar tentando baixar o modelo sem internet). "
                "Digite o código manualmente abaixo. O OCR volta automaticamente quando a internet retornar."
            )
        elif not EASYOCR_INSTALADO:
            st.warning("OCR não disponível. Verifique se 'easyocr' está em requirements.txt.")
        elif not EASYOCR_DISPONIVEL:
            st.warning("OCR não pode ser inicializado no momento. Digite o código manualmente.")
        else:
            st.info("Dica: Aponte a câmera para a etiqueta e tire a foto.")

        foto = None
        if not MODO_OFFLINE:
            foto = st.camera_input("Aponte a câmera para a etiqueta e clique em 'Take Photo'")
        else:
            st.caption("📷 Câmera desativada em modo offline. Use o campo abaixo para digitar o código direto.")
                ...
                if EASYOCR_DISPONIVEL:
                    with st.spinner("Analisando imagem com OCR..."):
                        ...
                        if texto_ocr:
                            ...
                            if codigo_detectado:
                                st.markdown(f'<div class="ocr-box"><h4>Código Detectado: <code>{codigo_detectado}</code></h4></div>', unsafe_allow_html=True)
                                st.session_state.codigo_ocr = codigo_detectado
                                st.session_state.busca_ativo = codigo_detectado
                            else:
                                st.markdown('<div class="warning-box"><h4>Código não reconhecido</h4></div>', unsafe_allow_html=True)
                        else:
        ...
        st.markdown("---")
        st.markdown("### Confirmar ou Digitar Código Manualmente")
        if st.session_state.get("limpar_busca_ativo", False):
            st.session_state.busca_ativo = ""
            st.session_state.limpar_busca_ativo = False
        busca = st.text_input("Número do Ativo:", placeholder="Ex: 00000333 ou 333", key="busca_ativo")
        ...
        if busca:
            ...
            if not item.empty:
                ...
                st.markdown(f'<div class="success-box"><h4>Item Encontrado</h4><p><strong>Código:</strong> {codigo_completo}</p><p><strong>Descrição:</strong> {descricao}</p><p><strong>Status:</strong> <span style="color: {cor_status}; font-weight: bold;">{status_atual}</span></p></div>', unsafe_allow_html=True)
                if status_atual == "Auditado":
                    ...
                    obs = st.text_area("Observações (opcional):", placeholder="Ex: Bom estado...", height=80, key="obs_auditoria")
                ...
                st.markdown('<div class="error-box"><h4>Item Não Encontrado</h4></div>', unsafe_allow_html=True)
                with st.expander("Cadastrar como Sobra"):
                    desc_sobra = st.text_input("Descrição da Sobra:", placeholder="Ex: Computador Dell", key="desc_sobra")
                    obs_sobra = st.text_area("Observações:", placeholder="Ex: Encontrado no escritório...", height=60, key="obs_sobra")
                    if st.button("Salvar como Sobra", key="btn_sobra"):
                        if desc_sobra:
                            if verificar_item_existe(busca, obra_id_atual):
                                st.error(f"⚠️ O código '{busca}' já existe nesta obra! Não é possível cadastrar duplicado.")
                            else:
                                data_agora = pd.Timestamp.now().strftime("%d/%m/%Y %H:%M")
        ...
        st.markdown("---")
        st.markdown("### Cadastrar Item Sem Patrimônio")
        st.markdown('<div class="sem-pat-box"><p>Cadastre itens <strong>sem etiqueta de patrimônio</strong>.</p></div>', unsafe_allow_html=True)

        with st.form("form_sem_patrimonio"):
            col_sp1, col_sp2 = st.columns(2)
            with col_sp1:
                desc_sem_pat = st.text_input("Descrição do Item:", placeholder="Ex: Mesa de escritório...")
            with col_sp2:
                qtd_sem_pat = st.number_input("Quantidade:", min_value=1, value=1, step=1)
            local_sem_pat = st.text_input("Local / Setor:", placeholder="Ex: Escritório 3...")
            obs_sem_pat = st.text_area("Observações:", placeholder="Ex: Item novo...", height=60)

            submitted = st.form_submit_button("CADASTRAR ITEM SEM PATRIMÔNIO", type="primary")
            if submitted:
                if desc_sem_pat and local_sem_pat:
                    ...
                    if verificar_descricao_existe(descricao_final, obra_id_atual):
                        st.error(f"⚠️ Já existe um item com a descrição '{desc_sem_pat}' nesta obra! Não é possível cadastrar duplicado.")
                    else:
                        data_agora = pd.Timestamp.now().strftime("%d/%m/%Y %H:%M")
                    ...
                    st.warning("Preencha Descrição e Local.")
