...
TIMEOUT_PRECHECK_REDE = 2.0

# ============================================================
# CONFIGURAR OCR (EasyOCR) - CARREGAMENTO PREGUICOSO (LAZY)
# ============================================================
# IMPORTANTE: nao instanciamos easyocr.Reader() aqui em cima. O Reader baixa os
# pesos do modelo na primeira vez que e usado - se isso rodar sem internet, o
# app TRAVA tentando alcancar a rede antes de falhar. Por isso so verificamos
# se a biblioteca esta instalada (checagem local, sem rede) e o Reader de
# verdade so e criado mais abaixo, sob demanda, e somente quando o app estiver
# online (ver _obter_leitor_ocr()).
try:
    import easyocr
...
_EASYOCR_READER = None  # sera preenchido por _obter_leitor_ocr(), so quando online
...
def _precheck_tcp_rapido(engine_remoto):
    """Tenta abrir um socket TCP no host:porta do banco com timeout curto
    (TIMEOUT_PRECHECK_REDE). Isso detecta 'sem rede nenhuma' (ex: usuario sem
    wifi/dados) rapido, em vez de esperar o timeout longo (10-30s) que a
    conexao completa do driver do banco usaria.

    Roda em uma thread separada com join(timeout=...) porque o timeout do
    socket.create_connection so cobre o connect - a resolucao de DNS que
    acontece antes dele NAO tem timeout garantido e, sem internet, pode travar
    por muito mais tempo que TIMEOUT_PRECHECK_REDE (isso e o que travava o
    campo de digitacao manual: cada tecla dispara um rerun do Streamlit, que
    rodava esse precheck, que ficava preso no DNS)."""
    resultado = {"ok": False}
    ...
    def _tentar():
        try:
            ...
            if not host:
                resultado["ok"] = True  # sem host para testar (ex: sqlite) - deixa a conexao real decidir
                return
    ...
    if t.is_alive():
        # DNS/conexao ainda presos alem do timeout - trata como offline agora.
        # A thread e daemon e morre sozinha quando o sistema operacional desistir;
        # nao bloqueia o app.
        return False
    return resultado["ok"]


def _tentar_conexao_completa_com_timeout(engine_remoto, timeout_segundos=3.0):
    """Roda engine_remoto.connect() + SELECT 1 numa thread separada com
    join(timeout=...). Nao confiamos so no 'connect_timeout' do driver porque
    ele pode nao ser respeitado igual em todo ambiente (SO, driver, proxy,
    WSL2, etc.) - isso e o que ainda podia deixar a tela travada/cinza por
    varios segundos na carga inicial mesmo com o pre-check TCP passando."""
...
def testar_remoto(engine_remoto):
    ...
    Duas camadas de protecao contra travamento:
    1) Pre-check TCP rapido (com timeout via thread) para detectar falta total
       de rede sem nem tentar o driver do banco.
    2) A tentativa de conexao completa TAMBEM roda com timeout via thread,
       porque o 'connect_timeout' do driver sozinho nao e garantia suficiente
       em todo ambiente."""
    ...
    return _tentar_conexao_completa_com_timeout(engine_remoto, timeout_segundos=3.0)
...
def criar_tabela_inventario(engine):
    ...
    try:
        with engine.begin() as conn:
            ...
            # IF NOT EXISTS: mesmo que duas sessoes/reruns do Streamlit cheguem
            # aqui "ao mesmo tempo" (ex: 2 abas abertas, ou rerun disparado
            # enquanto outro ainda esta rodando) e ambas passem pelo check acima
            # antes de qualquer uma criar a tabela, o CREATE em si nao quebra -
            # ele e uma operacao atomica e idempotente no proprio banco.
            if is_sqlite:
                conn.execute(text("""
        ...
        # "ja existe" nunca deve derrubar o app - se chegou aqui e porque outra
        # sessao/rerun ja criou a tabela (ou o indice) entre o check e o create.
        # Isso e um resultado aceitavel, nao um erro real.
        msg = str(e).lower()
...
# OCR so faz sentido (e so e seguro tentar) quando o app esta online: em modo
# offline, ele fica fora para nao arriscar travar tentando baixar modelo.
EASYOCR_DISPONIVEL = EASYOCR_INSTALADO and not MODO_OFFLINE


@st.cache_resource(show_spinner=False)
def _obter_leitor_ocr():
    """Cria o Reader do EasyOCR uma unica vez por sessao de servidor, e so
    quando ha internet (chamado apenas quando EASYOCR_DISPONIVEL e True).
    Isso evita a trava de inicializacao quando o app esta offline."""
    try:
        import easyocr
...
st.markdown("""
    ...
    /* Oculta a barra de ferramentas do Streamlit Cloud:
       ...
       Importante: NAO escondemos o [data-testid="stToolbar"] inteiro, pois em
       versoes recentes do Streamlit a setinha de abrir/fechar a sidebar fica
       dentro desse mesmo container. Se escondermos o container todo, a seta
       some junto. Por isso escondemos so os botoes de acao (stToolbarActions)
       e mantemos explicitamente a seta da sidebar visivel. */
    ...
    /* Garante que o botao de abrir/fechar a sidebar continue visivel,
       mesmo escondendo os outros icones da toolbar (compatibilidade com
       varias versoes do Streamlit). */
...
if MODO_OFFLINE:
    ...
    if _erro_conexao_atual:
        with st.expander("Detalhes tecnicos (por que esta offline)"):
    ...
    with tab1:
        ...
        if MODO_OFFLINE:
            st.warning(
                ...
                "Digite o codigo manualmente abaixo. O OCR volta automaticamente quando a internet retornar."
            )
            ...
            st.warning("OCR nao pode ser inicializado no momento. Digite o codigo manualmente.")
        else:
        ...
        if not MODO_OFFLINE:
            ...
            st.caption("📷 Camera desativada em modo offline. Use o campo abaixo para digitar o codigo direto.")
