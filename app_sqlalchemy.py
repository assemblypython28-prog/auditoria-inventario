...
def deletar_tudo_banco():
    """Deleta TODOS os registros do banco (compatível com CockroachDB)."""
    try:
        def _deletar(conn):
            # Usa DELETE em vez de TRUNCATE para compatibilidade com CockroachDB Serverless
...
# ============================================================
# FUNCOES DE LIMPEZA COMPLETA (APP + BANCO)
# ============================================================
def limpar_tudo_app_e_banco():
    """Limpa TUDO: banco de dados, session_state, memória do app."""
    # 1. Limpa banco de dados
    sucesso_banco = deletar_tudo_banco()

    # 2. Limpa TODAS as chaves do session_state relacionadas a dados
    chaves_para_remover = []
    ...
    # 3. Recria o estado inicial limpo
...
# ============================================================
# INICIALIZACAO DO SESSION STATE (MULTISESSAO / MULTIOBRA)
# ============================================================
# Usa chaves por obra_id para suportar multisessão simultânea
...
# Chave única para dados da obra atual no session_state
...
# Flags para confirmação de exclusão (2 etapas, sem digitar texto)
...
with st.sidebar:
    ...
    # Input de obra com key única para evitar conflitos
    ...
    # Se mudou a obra, atualiza o session_state e recarrega
    if obra_input != obra_id_atual:
        st.session_state.obra_id = obra_input
        # Cria chave nova para a nova obra
    ...
    # SEMPRE recarrega do banco antes de exportar para garantir dados atualizados
    ...
    # Botão para limpar apenas a tela (memória local da obra)
    ...
    # ============================================================
    # EXCLUIR OBRA DO BANCO (SEM DIGITAR TEXTO - 2 CLIQUES)
    # ============================================================
    ...
    # ============================================================
    # EXCLUIR TUDO (APP + BANCO) - SEM DIGITAR TEXTO - 2 CLIQUES
    # ============================================================
...
# ============================================================
# AREA PRINCIPAL - DADOS DA OBRA ATUAL
# ============================================================

# Recarrega do banco para garantir dados frescos (multisessão)
            ...
            if not item.empty:
                ...
                if status_atual == "Auditado":
                    ...
                    if st.button("CONFIRMAR AUDITORIA", type="primary", key="btn_auditoria"):
                        ...
                        if sucesso:
                            # Atualiza o DataFrame local e o banco
