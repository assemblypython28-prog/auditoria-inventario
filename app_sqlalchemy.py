...
def _precheck_tcp_rapido(engine_remoto):
    ...
    wifi/dados) em ~2s, em vez de esperar o timeout longo (10-30s) que a
    conexao completa do driver do banco usaria. Se o precheck falhar, ja
    sabemos que estamos offline sem precisar tentar a conexao de verdade."""
    try:
        url = engine_remoto.url
        host = url.host
        porta = url.port or (5432 if "postgres" in url.drivername else 26257)
        if not host:
            return True  # sem host para testar (ex: sqlite) - deixa a conexao real decidir
...
def testar_remoto(engine_remoto):
    ...
    Primeiro faz um pre-check TCP rapido (poucos segundos) para detectar falta
    total de rede sem precisar esperar o timeout longo da conexao completa -
    isso e o que evitava o app 'travar' por 10-30s quando o usuario estava
    totalmente sem internet."""
